...
//   float3 ambienteMasDifusa=saturate(ambiente+difusa);
   output1.ColorDifuso= float4 (difusa,1);
...
float4 pixelShaderEspecular (float4 especul:COLOR0,float4 dif: COLOR1,float2 tex0:TEXCOORD0,float3 ambiente:TEXCOORD1):COLOR 
{
   float3 texColor=tex2D (muestra1,tex0).rgb;
