...
float4 psBlinnPhong(VS_OUTPUT2 IN):COLOR
{
   ...
   float3 color=(saturate(ambient+diffuse)*texColor+specular)*luzDifusa+0.15*texColor;//float3 color=(saturate(ambient+diffuse)*texColor+specular)*luzDifusa; 
...
VS_OUTPUT1 vertexShaderEspecularDifusaAmbiente (VS_INPUT input1)
{
   ...
   float3 posOjoTransf= normalize(posOjo.xyz-posVertice);
   ...
   float s=max(dot(vecDirLuz,normalTransf),0.0f);
   float3 difusa=s*luzDifusa.rgb*matDiffuse;
   ...
   //output1.ColorDifuso= float4 (difusa,0);
...
float4 pixelShaderEspecular (float4 especul:COLOR0,float2 tex0:TEXCOORD):COLOR  //,float3 ambiente:TEXCOORD1,float4 dif: COLOR1,
{
   float3 texColor=tex2D (muestra1,tex0).rgb;
//   float3 difuso=texColor;//+dif+especul;      //saturate(texColor*(ambiente.rgb+dif.rgb))+especul.rgb;//saturate(texColor*c);
   return especul;//float4(difuso,dif.a);
}
...
technique RenderizarEspecularDifusaAmbiente
{
   pass Pass_0
   {
      ...
      PixelShader =compile ps_2_0 pixelShaderEspecular();
   }
}
