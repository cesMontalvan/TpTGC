...
VS_OUTPUT2 vsBlinnPhong(VS_INPUT IN)
{
   ...
   float4 worldPos=mul(IN.Position,matWorldViewProj);
...
float4 psBlinnPhong(VS_OUTPUT2 IN):COLOR
{
   ...
   //float3 diffuse= saturate(dot(normal,light))*matDiffuse; 
   ...
   float alpha=matDiffuse*texColor.a;
...
float4 pixelShaderEspecular (float4 especul:COLOR0,float4 dif: COLOR1,float2 tex0:TEXCOORD):COLOR  
{
   float3 texColor=tex2D (muestra1,tex0).rgb;
