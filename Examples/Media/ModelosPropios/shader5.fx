...
float4 pixelShaderEspecular (float4 especul:COLOR0,float2 tex0:TEXCOORD,float3 ambiente:TEXCOORD1,float4 dif: COLOR1):COLOR  //
{ 
   float3 texColor=tex2D (muestra1,tex0).rgb;
   float3 difuso=saturate(texColor*(ambiente.rgb+dif.rgb))+especul.rgb;//saturate(texColor*c);
