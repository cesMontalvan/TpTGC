...
VS_OUTPUT vs_main( VS_INPUT Input )
{
   ...
   float3 dirLuz=mul(float4(fvLightPosition,0),matWorldViewProjection);               //agregado
   float3 ojoPos=mul(float4(fvEyePosition,0),matWorldViewProjection);                  //agregado

   Output.Position = mul( Input.Position, matWorldViewProjection );
   Output.Texcoord = Input.Texcoord;
   
   float3 fvObjectPosition = mul( Input.Position,matWorldViewProjection);    //matEspecial);//matWorld );//
...
{ 
   ...
   float3 fvNormal1        = mul( tex2D( bumpMap, Input.Texcoord ), matWorld);//matWorldViewProjection);
   float3 fvNormal         = mul(fvNormal1,matWorldViewProjection);
